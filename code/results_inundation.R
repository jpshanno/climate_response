...
proportions <- 
  wetland_simulation_summaries[, .SD, .SDcols = patterns("proportions")] %>% 
  setnames(function(x){str_remove(x, "proportions.")}) %>% 
  setnames(function(x){str_replace(x, "prop", "prop_above")}) %>% 
  setnames(function(x){str_replace(x, "prop_above_([1-9])", "prop_above_neg_\\1")})
