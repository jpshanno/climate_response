...
model_pet_drawdown <- 
  function(data,
    ...
    data[, morphology_flow_cm := morpho.models[.BY[[1]], f_predict[[1]]](wl_initial_cm, outflow.only = FALSE),
         by = .(site)]
    
    # Calculate expected meteorology net flow
    data[, meteo_flow_cm := meteo.models[CJ(.BY[[1]], .BY[[2]]), f_predict[[1]]](pet_cm, water_availability_cm),
         by = .(site, site_status)]
    
    # Calculate expected precip rise
    data[, precip_rise_cm := rise.models[.BY[[1]], f_predict[[1]]](net_precip_cm),
         by = .(site)]
    
    # Determine remaining drawdown from PET
    data[, drawdown_cm := (morphology_flow_cm - meteo_flow_cm) - Ds_cm + precip_rise_cm]
    ...
    mods <- 
      ...
      future_map(~.x[, .(site_status = first(site_status),
                         mod = list(rlmerRcpp(drawdown_cm ~ pet_cm + (1 + pet_cm | site),
                                              data = .SD,
                                              method = "DASvar")))]) %>% 
      rbindlist() %>% 
      setkey("site_status")
    ...
    #        .(mod_crossed = list(lmrob(drawdown_cm ~ pet_cm,
    #                                   setting = "KS2014",
    #                                   data = .SD))),
    #        keyby = .(site)]
    # 
    ...
    setnames(fixed_pred,
             c("site_status", "mod", "intercept", "slope"))
    
    # PET range is already available in mod_meteo_flow
    ...
    test_predict <- 
      data[, fixed_pred[.BY[[1]], f_predict[[1]]](pet_cm),
           by = .(site_status)]$V1
    ...
    random_pred <- 
      random_pred[fixed_pred[, -c("f_predict")], 
                  on = "site_status"]
    ...
    test_rand_predict <- 
      data[, random_pred[CJ(.BY[[1]], .BY[[2]]), f_predict[[1]]](pet_cm),
           by = .(site, site_status)]$V1
