...
relationship between modeled and modeled values, but it cannot be used to
evaluate the accuracy of predictions [@krause-2005]. We use the median
prediction error to determine model predictive accuracy. In addition to the
overall accuracy of the models we want the models to represent the hydrologic
dynamics of interest (inundation, connectivity, and drawdown). We tested the
predicted probabilities of inundation, connectivity, and drawdown to the
observed probabilities of the same conditions.
...
results by @moriasi-2015 (Figure EEE). Site 135 had much higher errors than all
other sites excluding 113. Results from wetland model performance
and simulation data from these sites have been excluded from further analysis
due to consistently the poor performance in the test data. Based on knowledge of
these sites the authors believe these sites would require different hydrologic
process models. Site 119 is a closed basin with no surface outlet and was shown
to have different Site 113 has multiple defined seeps which provide defined
surface water inputs to the site. source water characteristics than other sites 
[@vangrinsven-2017]. Site 135 is the driest of all study sites and unlike other
sites has only infrequent and localized surface water.
...
knitr::include_graphics("output/figures/boxplot_wetland_model_boxplot.tiff")
...
tar_read(wetland_model_predicted_probs_table) %>%
...
**FIG AAA**. The LOCA data represents 30 years of
data compared to the 10,000 years of synthetic weather series. We may expect more outliers in the SWG data than in
the LOCA data because the quantity of observations will increased samples in the
tails of distributions. To compare the
...
were compared to the distribution of LOCA values. **Fig BBB A** shows good
agreement for all seasons for daily minimum and maximum temperature. The 
distribution of simulated monthly
precipitation totals are shown in **Fig BBB B**. The 30 sets of 30-year
synthetic weather series show variability relative to LOCA values, but cluster 
around the LOCA value as a mean.
...
```{r swg-check-plot, fig.cap="Density plots of 30 years of LOCA generated data, and 30 random samples of 30-year periods from the full 10,000 years of SWG synthetic weather series."}
...
and greater than the less sensitive scenario. The less sensitive scenario had
a higher probability of drawdown conditions than observations, but 
showed overlap with interannual variation. Drawdown events were very unlikely
under replacement forested conditions.
