...
wetland conditions than were observed (Table \@ref(tab:model-metrics)). Both the 
median rRMedSE is moderate for both Control and Treated condition models (~10%),
but notable outliers were present (wetland 053) (Table \@ref(tab:model-metrics)).
...
\@ref(fig:model-metrics)A, Table \@ref(tab:model_metrics)). We found that 91% of
...
probability of occurrence of *CEHT* levels. Table \@ref(tab:predicted-probs) and
...
analysis, Table \@ref(tab:model-metrics) does indidcate a bias towards drier
...
sites can incorpoated without resulting in unbalanced vegetation-conditions
...
*CEHT* is not symetrical around the median, indicating for instance that the 
...
dat[i = sample_season == "JJA", j = lapply(.SD[,c("precip_cm", "pet_cm")], sum), by = .(Climate, sample_year)][, deficit := precip_cm - pet_cm][, ggdist::median_hdci(.SD, precip_cm, pet_cm, deficit, .width = 0.67), by = .(Climate)]

-->

The future climate scenarios used in this study present two alternative future
scenarios. The less sensitve scenario summer is projected to be slightly drier
than observed conditions, with similar *PET* (Table \@ref(tab:future-climate).
However, when summer water defecit is calculated as the *P-PET* we see that with
...
and future climate (Table \@ref(tab:future-climate). There is little to no
...
both future scenarios, with more dramatic impacts under the more senstive 
scenario. This shift will likely have the result of increasing water defecit on
the landscape relative to the values presented in Table \@ref(tab:future-climate).

```{r future-climate-table}

tar_read(future_climate_table) %>%
flextable::autofit() %>% 
flextable::set_caption(
    caption = "Summer (JJA) precipitation, potential evapotranspiration, and water deficit (P - PET) for the climate scenarios in this study. Future climate data were taken from LOCA downscaled daily values, and observed climate was taken from observed conditions at Bergland Dam, MI, USA from 1980-2009. Reported values are the median and bounds of the 67% highest density continous interval of seasonal total precipitation by observed/projection year.",
...
difference [@mclaughlin-2017]. The effect is that *PET* results in smaller changes
...
<!-- TODO: refrence these from intro "3) site conditions following EAB infestation, and 4) site conditions in a future climate." -->
