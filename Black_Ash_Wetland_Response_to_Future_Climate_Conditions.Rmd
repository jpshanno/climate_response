...
(RMedSE) to assess overall predictive accuracy. In addition to the
...
For the remaining sites 79.59% of site-year combinations had *R^2* values above
0.75 (Figure \@ref(fig:model-metrics)). Wetland models tended to predict drier
than observed conditions, with median errors ranging from -15.56--6.38 cm (median ), with
77.56% of site-year combinations have a negative median error 
(Figure \@ref(fig:model-metrics)). The RMedSE ranged from 1.77--18.51 cm with a 
median of 4.62 cm. When RMedSE is normalized by the annual range of water level 
fluctuation within a wetland (rRMedSE) errors range from 3.67--78.68% of annual 
water level range with a median value of 13.64%.
...
```{r predicted-probs, fig.cap="Observed and wetland model predicted probability of hydroecologically-relevation water levels."}
...
levels. Model performance varied by site but median rRMedSE was 13.64% of annual
water level range and median model error was -2.62 cm. In this case daily
percent bias estimates (PBIAS) would be helpful to consider error relative to
observed daily water level. However, wetland water levels cross 0 so that the
relative error approaches infinity as water levels rise or fall towards the
surface.
...
forest. Non-linear response in wetland plant communties affecting model results [@short-2016]
